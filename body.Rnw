...
#per.rep.merg <- per.rep.merg[order(per.rep.merg$tagnum),
#c("GeneFamilyName","SALK_Line","tagnum","fruits.removed",
#                                     "alive.at.harvest" ,
#                                     "treat","percent","Gene_Family","Sub_Family",
#                                     "gene.family.size","cntrl")]
        ...
        if (rws<makereps)
            {
               ...
               newdf$abortn <- 0
               newdf$alive.at.harvest <- NA
               newdf$silique <- NA
               newdf$mass <- NA
               fitmerg0 <- rbind(fitmerg0,newdf)
            }
    }
fitmerg0 <- fitmerg0[fitmerg0$SALK_Line!="SALK_008897C",] #no germination at all
...
<<fig1-compare-to-control,echo=F,fig.keep='high',fig.width=8,fig.height=6.5,dev=c('pdf','CairoPNG'),cache=F>>=
comp2ctrl(fitmerg)
@ 
...
<<calc.gls,echo=F,results="hide",cache=T,eval=F>>=
...
<<stand.resid,echo=F,out.width="4in",dev=c('pdf','CairoPNG'),eval=F>>=
...
These data do include the non-germinated seed categories.  It is
definitely these zeros that cause all the residual behavior described
above.
...
par(mfrow=c(2,1))
hist(fitmerg.nz$fitness,main="Data without the non-germinants added")
hist(fitmerg$fitness,main="Data with the non-germinants added")
@ 


<<glmfits,echo=F>>=
fit.glm <- glm(fitness~SALK_Line,data=fitmerg)
fit.glm.pois <- glm(round(fitness)~SALK_Line,data=fitmerg,family="poisson")
@ 


So rather than relying on the ridiculously over-parameterized
independent variance models above, I decided to bite the bullet and
start working with zero-inflated models.
...
this a mixture model might be the way to go.  Two such models (zero
inflated poisson and zero inflated neg binomial) are fit to these data
and the ad-hoc test below indicates that the negative binomial works
better.
...
<<zero-resid,echo=F,out.width="4in",echo=F,dev=c('pdf','CairoPNG')>>=
plot(resid(fit.zero.pois)~fitted(fit.zero.pois))
@ 

So we can perform a means-comparisons analysis like the ones described
above for the zero-inflated data.  Things are a little more
complicated for a couple of reasons: 1) there are actually two models
fit, a count model and an inflation model.  We could interpret them as
the effect of line on the number of fruit after germination and the
effect of line on germination.  This latter model is a little
troubling though, because the germination rates are correlated due to
common germination pot. I don't put much stock in it.  2) there are
not really post-hoc tests developed for zero-inflated models so the
bonferroni correction on contrasts is the only real game in town.

When using the bonferroni correction method, there are 20 lines below
(17\%) and 7 lines greater (6\%).  So lower than the mean-focused
approach, but still nearly 1/4 show ``phenotypes''.

<<echo=F>>=
cnts <- data.frame(summary(fit.zero.nb)$coefficients[[1]])
zeros <- data.frame(summary(fit.zero.nb)$coefficients[[2]])
names(cnts) <- c("Estimate","StdErr","z","p.value")
names(zeros) <- paste("zero",names(cnts),sep=".")
cnts$source <- rownames(cnts)
zeros$source <- rownames(zeros)
cnts <- cnts[-c(1,dim(cnts)[1]),]
zeros <- zeros[-c(1,dim(zeros)[1]),]
coef.lm <- merge(cnts,zeros)
coef.lm$bonsig <- coef.lm$p.value<(0.05/115)
coef.lm$sign <- ifelse(coef.lm$Estimate<0,"below","above")
coef.lm$zero.bonsig <- coef.lm$zero.p.value<(0.05/115)
coef.lm$zero.sign <- ifelse(coef.lm$zero.Estimate<0,"below","above")
@ 

<<zero-cnt-hist,echo=F,dev=c('pdf','CairoPNG')>>=
hist(coef.lm$z,main="Histograms of effect sizes for counts")
lowbon <- max(coef.lm$z[(coef.lm$bonsig)&(coef.lm$z<0)])
hibon <- min(coef.lm$z[coef.lm$bonsig&(coef.lm$z>=0)])
abline(v=lowbon,col="blue")
abline(v=hibon,col="blue")
with(coef.lm,table(bonsig,sign))[2,]
@ 

<<zero-zero-hist,echo=F>>=
hist(coef.lm$zero.z,main="Histograms of effect sizes for counts")
#lowbon <- max(coef.lm$zero.z[(coef.lm$zero.bonsig)&(coef.lm$zero.z<0)])
hibon <- min(coef.lm$zero.z[coef.lm$zero.bonsig&(coef.lm$zero.z>=0)])
#abline(v=lowbon,col="blue")
abline(v=hibon,col="blue")
@ 

<<echo=F>>=
coef.lm$SALK_Line <- gsub("SALK_Line","",coef.lm$source)
write.csv(file="zero-inf.csv",row.names=F,coef.lm)

@ 

\subsection{Posthoc power analysis--zero inflated model}
What follows is basically the same power analysis performed above on the gls models.

<<echo=F>>=
reptbl <- data.frame(table(fitmerg$SALK_Line))
names(reptbl) <- c("SALK_Line_char","reps")
reptbl$SALK_Line_char <- as.character(reptbl$SALK_Line_char)
fitmerg <- fitmerg[,names(fitmerg)!="reps"]
fitmerg$SALK_Line_char <- as.character(fitmerg$SALK_Line)
fitmerg <- merge(fitmerg,reptbl)

if (!file.exists("powerdat-zero.rda"))
    {
        models <-do.call(rbind, mclapply( rep(c(2:10),1000),
                           mc.cores=3,mc.preschedule=F, 
                           function(ss)
                           {
                               print(ss)
                                        #        lowreps <- fitmerg[fitmerg$reps<=10,]
                               tmpmerg <- fitmerg[fitmerg$reps>ss,] %>% group_by(SALK_Line) %>% do(sample_n(.,ss))
                               linemax <- tmpmerg%>%group_by(SALK_Line)%>%summarise(lmax=max(fitness))%>%filter(lmax>0)
                               tmpmerg <- tmpmerg[as.character(tmpmerg$SALK_Line)%in%as.character(linemax$SALK_Line),]
                               ...
                               tmp <- tryCatch(
                                   data.frame(summary(zeroinfl(round(fitness)~SALK_Line, dist="negbin", data=tmpmerg))$coefficients[[1]]),
                                               error=function(r){NULL})
                               if (is.data.frame(tmp))
                                   {
                                       names(tmp) <- c("Estimate","StdErr","z","p.value")
                                       tmp <- tmp[-c(1,dim(tmp)[1]),]
                                       tmp$bonsig <- tmp$p.value <= 0.05/dim(tmp)[1]
                                       tmp$sign <- ifelse(tmp$z>0,"above","below")
                                       c("ss"=ss,table(tmp$bonsig,tmp$sign)[2,]/dim(tmp)[1])
                                   }
                           }))
        save(file="powerdat-zero.rda",models)
    }else{load("powerdat-zero.rda")}


@ 

The following two plots show the relationship between the number of
plants grown and the ability to detect the proportion of plants that
have fitness above and below the control.  Note the inflection at a sample size of 7 for the plants with fitness greater than controls.


<<power-above,echo=F,dev=c('pdf','CairoPNG'),out.width="4in">>=
xyplot((jitter(above,500)+0.001)~ss,type=c("p","smooth"),data=models,scales=list(y=list(log=T)),
       xlab="Sample Size",ylab="Log(proportion lines sig above control)")
@ 

<<power-below,echo=F,dev=c('pdf','CairoPNG'),out.width="4in">>=
xyplot((jitter(below,500)+0.001)~ss,type=c("p","smooth"),data=models,scales=list(y=list(log=T)),
        xlab="Sample Size",ylab="Log(proportion lines sig above control)")
@ 
...
Here is a comparison of all the fitness components pairwise.
IMPORTANT: I realized in the past day that we do have silique length
information.  I'm not sure how it was taken, there is only a single
measurement per plant.  Not sure if this is a haphazardly selected
silique, the lowest on the inflorescence, or what.  They are not means
of multiple measures because they always have a 0 or 5 after the
decimal point (8.5, 6.0, etc).  

<<splom-fitness-traits,echo=F,fig.keep='high',fig.width=11,fig.height=11,dev=c("pdf","CairoPNG"),comment=NA>>=
...
fitline.means <- rbind(fitline.means[fitline.means$SALK_Line!="SALK_CS60000",],
                       fitline.means[fitline.means$SALK_Line=="SALK_CS60000",])
...
pairs(fitline.means[,-1], lower.panel = panel.cor, upper.panel = panel.smooth)
...
<<>>=
lns[which(lns%in%fitmads$SALK_Line)]
@ 
