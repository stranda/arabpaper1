...
suppressMessages({if (!(require(Hmisc)&require(lme4)&require(lattice)&require(compiler)&require(car)))
    {
...
<<theory-predict,dev=c('pdf','postscript','CairoPNG'),echo=F,fig.keep='none',out.width='"0.6\\fullwidth"',comment=NA>>=
#xfig("theory.fig")
d=seq(0.00,1,0.01)
exp.height=dexp(0,10)/100
plot(dbeta(sort(d,decreasing=T),1.9,6.4)~d,type="l",axes=F,xlab="Fitness",ylab="", xlim=c(0,1.5),lwd=3, cex.lab=1.5)
mtext("Frequency of mutant lines",2,1,cex=1.5)
mtext("Ancestor's\nfitness",1,1.3,at=1,cex=1.2)
mtext("Low",1,0.5,at=0.1,cex=1.2)
mtext("High",1,0.5,at=1.4,cex=1.2)
mtext("Distribution of mutant lines' fitness\nrelative to ancestor",3,1,cex=1.7)
box()
@ 
...
<<compare-to-control3,echo=F,fig.keep='none',dev=c('pdf','postscript','CairoPNG'),comment=NA,cache=F>>=
...
avg.reps <- mean(fitmerg$reps[(fitmerg$reps>1)&(as.character(fitmerg$SALK_Line)!="SALK_CS60000")])
fitline <- with(fitmerg[(fitmerg$reps>1),],aggregate(fitdev,by=list(SALK_Line=SALK_Line),mean))
###fitline <- with(fitmerg[(fitmerg$reps>1)&(as.character(fitmerg$SALK_Line)!="SALK_CS60000"),],aggregate(fitdev,by=list(SALK_Line=SALK_Line),mean))
...
for (i in 1:numreps)
  {
    cntrlmeans[i] <- mean(
                          sample(fitmerg$fitdev[as.character(fitmerg$SALK_Line)=="SALK_CS60000"],
                                 size=round(avg.reps)
                                 )
                          )
  }
cntrls <- fitmerg$fitdev[as.character(fitmerg$SALK_Line)=="SALK_CS60000"]

#
#make the figure
#

addq <- function(quant,ctmn,ctsd)
  {
...
superhist2(list(fitline[,2],cntrlmeans,cntrls),main="Difference in fitnesses and control mean",nbreaks=10,ylab="",xlab="Fitness deviation from control",cex.lab=1.5,cex.axis=1.3)
...
<<fig1-compare-to-control,echo=F,fig.keep='high',fig.width=8,fig.height=6.5,dev=c('pdf','CairoPNG'),comment=NA>>=
...
<<fitmads,echo=F,dev=c('pdf','postscript','CairoPNG')>>=
...
<<fitmads2,echo=F,dev=c('pdf','postscript','CairoPNG')>>=
bwplot(fitdev~SALK_Line,data=fitmads,horizontal=T,ylab="Fitness deviation from control",xlab="MADS lines")
...
#write.table(file="MADS-function.csv",sep=",",row.names=F,mnmads)
dotplot(SALK_Line~fitdev,mnmads)
@ 



<<compare-to-control-back2back,echo=F,fig.keep='none',dev=c('pdf','postscript','CairoPNG'),comment=NA>>=
...
<<fig-s3-fitness-components,echo=F,fig.keep='high',fig.width=7,fig.height=4,dev=c("pdf","CairoPNG","postscript"),comment=NA>>=
#plot(fitline.alldata[,c("fitness","flowers","fruitnum","mass","percent","silique","abortn")])
fitline.means <- with (fitline.alldata,aggregate(cbind(fitness,flowers,fruitnum,mass,abortn,percent,silique,alive.at.harvest),by=list(SALK_Line=SALK_Line),mean,na.rm=T))
indic=1+(fitline.means$SALK_Line=="SALK_CS60000")
#plot(fitline.means[,-1],pch=c(1,15)[indic],col=indic,cex=c(1,1.4)[indic])
...
<<fig-s1-tdna-dist,echo=F,fig.keep='high',dev=c("CairoPNG","postscript","pdf"),comment=NA>>=
...
<<fig-s2-TDNA-ratio-plot,echo=F,fig.keep='high',dev=c("CairoPNG","postscript","pdf"),comment=NA>>=
...
<<fig2-gene-fam-vs-fitdev,echo=F,fig.keep='high',dev=c("pdf","postscript","CairoPNG"),fig.width=3.5,fig.height=3.5,comment=NA>>=
par(mar=c(5,5,1,1)+0.1)
...
<<fig3-reaction-norm1-fit,fig.display='high',fig.width=10,fig.height=10,echo=F,dev=c("pdf","postscript","CairoPNG"),comment=NA>>=
...
<<fig-s4,fig.display='high',echo=F,dev=c("pdf","postscript","CairoPNG")>>=
par(mfrow=c(2,1))
...
<<germ-vs-fam,echo=F,dev=c('pdf','postscript','CairoPNG'),comment=NA>>=
