...
suppressMessages({if (!(require(Hmisc)&require(lattice)&require(compiler)&require(car)&require(multcomp)&require(dplyr)&require(nlme)&require(lme4)))
    {
...
if (.Device %in% c("pdf","png","CairoPNG"))
  {
    legend(x=19,y=y*1.4,
         c("Resampled controls",paste("Means of",length(unique(fitmerg$SALK_Line))-1,"Salk lines")),
...
the significance of these t-stats using Bonferroni.  There is one line
to the left of the blue interval and 11 to the right of the blue
interval(?!).  The interval among the red line corresponds to no
correction for multiple tests. In this case, 30 lines are lower than
the red interval and 22 are larger.

<<contrast-comps,echo=F>>=
#fitmerg holds the data 
fitmerg$SALK_Line <- as.factor(fitmerg$SALK_Line)
fitmerg$SALK_Line <- relevel(fitmerg$SALK_Line,"SALK_CS60000")
fit.lm <- lm(fitness~SALK_Line,data=fitmerg)

coef.lm <- summary(fit.lm)$coefficients
...
is Dunnett's test.  It corrects for multiple tests.

<<echo=F,cache=T>>=
fit <- aov(fitdev~SALK_Line,data=fitmerg)
m1 <- diag(1,120,120)
K <- as.matrix(cbind(rep(-11,120),m1))
fit.dunnetts <- glht(fit,linfct=mcp(SALK_Line="Dunnett"))
...
\subsubsection{Specific comparisons among a-priori-chosen subsets of genes}
...
<<compare-to-control-back2back,echo=F,fig.keep='none',dev=c('pdf','CairoPNG'),comment=NA>>=
